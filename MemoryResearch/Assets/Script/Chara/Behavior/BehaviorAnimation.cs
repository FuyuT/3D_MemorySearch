...
public static class BehaviorAnimation
{
    ...
    /// <returns></returns>
    public static bool IsNameCheck(ref Animator animator, string animName, int layerNo = 0)
    {
        if (!animator.GetCurrentAnimatorStateInfo(layerNo).IsName(animName))
        {
            return false;
    ...
    /// <param name="layerNo"></param>
    /// <returns></returns>
    public static bool IsPlayEndCheck(ref Animator animator, string animName, int layerNo = 0)
    {
        //現在再生しているアニメーションが、確認したいアニメーションと違う場合は終了
        if (!IsNameCheck(ref animator, animName, layerNo)) return false;
